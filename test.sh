...
for version in "${meteor_versions[@]}"; do
	printf "${YELLOW}Testing Docker image jorgenvatle/meteor-base:${version}...${NC}\n"
	SECONDS=0
	...
	run_with_suppressed_output "docker run --rm --volume ${PWD}:/opt/tmp --workdir /opt/tmp jorgenvatle/meteor-base:${version} meteor create --release=${version} test-app"

	cp "${dockerfile}" test.dockerfile
	do_sed "s|FROM jorgenvatle/meteor-base:.*|FROM jorgenvatle/meteor-base:${version}|" test.dockerfile
	...
	if [ $exit_code -ne 0 ]; then
		...
		if [[ $(get_version_string "${node_version}") -ge $(get_version_string 14.21.4) && $(get_version_string "${node_version}") -lt $(get_version_string 18.0.0) ]]; then
			printf "${RED}FAIL for jorgenvatle/meteor-base:${version} with meteor/node:${node_version}-alpine3.17${NC} after ${elapsed}\n"
		else
			printf "${RED}FAIL for jorgenvatle/meteor-base:${version} with node:${node_version}-alpine${NC} after ${elapsed}\n"
		fi
		at_least_one_failure=true
	else
		# For 14.21.4 <= $node_version < 18.0.0, we need to use the Meteor fork of the Node Docker image; else, we use the regular official Node Docker image
		if [[ $(get_version_string "${node_version}") -ge $(get_version_string 14.21.4) && $(get_version_string "${node_version}") -lt $(get_version_string 18.0.0) ]]; then
			printf "${GREEN}PASS for jorgenvatle/meteor-base:${version} with meteor/node:${node_version}-alpine3.17${NC} after ${elapsed}\n"
		else
			printf "${GREEN}PASS for jorgenvatle/meteor-base:${version} with node:${node_version}-alpine${NC} after ${elapsed}\n"
		fi
	fi
